...
name: supersede
description: >
  Mark a past decision in the lore as superseded or obsolete. Use whenever the
  ...
  decision dead", or invokes "/lore:supersede". Does NOT capture a new
  discussion; that belongs to the capture skill.
---

# Lore -- Supersede

Retire an existing lore entry by marking it `superseded` (with a pointer to its
...
1. **Find the target entry.** Read `lore/INDEX.md` and locate the entry by slug
...
5. **Update `lore/INDEX.md`.** Change the target entry's status column in place:
