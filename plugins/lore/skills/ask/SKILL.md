...
description: >
  ...
  things were implemented a certain way by searching the team's lore. Use this
  ...
  or invoke "/lore:ask". Trigger even when the user doesn't name the lore
...
exist.** If the lore has nothing on the topic, say so -- a confident answer
assembled from nothing is worse than "I didn't find anything about that."

## Storage layout

The lore lives under a repo-relative `lore/` directory:
...
If `lore/` or `INDEX.md` does not exist, tell the user the lore is empty or
...
2. **Shortlist from `lore/INDEX.md` first.** Read `lore/INDEX.md` and select
   ...
   - If `INDEX.md` is missing, looks stale, or the shortlist comes up empty but
     ...
     `lore/log/*.md` (search `areas:`, `topics:`, `stories:`, and titles).
...
4. **Filter to active; follow chains.** Answer only from active entries by
   ...
   YYYY-MM-DD and there is no current entry -- the lore has no current answer for
   this topic."
...
5. **Synthesize a focused answer.** Lead with the direct answer to what they
   ...
   > Per *RLS for tenant isolation* (2026-06-23, `lore/log/2026-06-23-rls-tenant-isolation.md`): ...
...
6. **Be honest about gaps.** If you find nothing relevant, say so directly:
   "I didn't find anything in the lore about <topic>." When close-but-not-exact
...
7. **Surface open questions.** After your answer, read `lore/QUESTIONS.md`
   ...
   **Still open in this area:** Q-001 (asked 2026-06-23, international-shipping):
   ...
   to populate this section without opening individual `lore/questions/Q-NNN.md`
...
Action: no matching entries. Respond: "I didn't find anything in the lore about
