...
name: capture
description: >
  Capture a technical discussion or meeting summary into the team's lore. Use
  ...
  later", or invoke "/lore:capture". Trigger even if the user only pastes a chunk
...
Turn a raw or AI-generated discussion summary into one compact, retrievable lore
...
All entries live under a repo-relative `lore/` directory:
...
If `lore/` does not exist yet, create it along with `lore/log/`,
`lore/questions/`, an empty `QUESTIONS.md`, a starter `topics.md`, and an
`INDEX.md`. Write this one-line header at the very top of the new `INDEX.md`:
```
# Lore: decision history for this repo
```
Never invent a second lore location -- if the user's repo already has `lore/`, use it.
...
4. **Tag on two facets -- areas and topics -- from the controlled vocabulary.**
   Read `lore/topics.md` first; it has two sections:
   ...
   Uncontrolled tags silently fragment the lore (`auth` vs `authentication` vs
...
6. **Check for supersession.** Before writing the new file, scan `lore/INDEX.md`
...
7. **Write the file** as `lore/log/YYYY-MM-DD-short-slug.md`. Make the slug
...
7b. **Register open questions.** For each genuine unresolved question in the
    ...
    - Determine the next Q-ID by reading `lore/QUESTIONS.md` (find the highest
      existing Q-NNN and increment). If `lore/QUESTIONS.md` doesn't exist yet,
      start at Q-001 and create the file with a `# Questions` heading.
    - Prefix the item in the lore entry with the ID: `- Q-001: How should we...`
    - Create `lore/questions/Q-NNN.md` with this exact frontmatter (leave
      ...
      ---
      ```
    - Append one line to `lore/QUESTIONS.md`:
      ```
...
7c. **Resolve any open questions this discussion answers.** If
    `lore/questions/` exists and contains files with `status: open`:
    ...
    - For each confirmed resolution:
      - Edit the `Q-NNN.md` frontmatter: set `status: resolved`, fill in
        ...
        entry's slug.
      - Update the corresponding line in `lore/QUESTIONS.md`: change `open` to
        `resolved`.
      - Note the closure in the report-back (step 9): "Closed: Q-002, Q-005."
    - If no candidates match, skip silently.

8. **Update `lore/INDEX.md`.** Append one line so the `ask` skill can shortlist
...
Output: `lore/log/2026-06-23-rls-tenant-isolation.md` with `areas: []`,
...
question becomes an individual file in `lore/questions/` (e.g. `Q-001.md`) and a
one-line entry in `lore/QUESTIONS.md`, so the next discussion can close them out
explicitly.
...
Output: two files -- `lore/log/2026-06-24-order-pdf-renderer.md`
(areas: [order-process], topics: [pdf, rendering]) and
`lore/log/2026-06-24-buyer-price-suggestion.md`
