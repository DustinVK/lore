---
description: Inspect the codebase and propose starter areas + topics for the lore
argument-hint: [optional focus path]
allowed-tools: Read, Grep, Glob, Bash(ls:*), Bash(find:*), Bash(cat:*), Bash(git:*), Write
---

You are bootstrapping the controlled vocabulary for this project's lore --
the two-facet `lore/topics.md` used by the `capture` and `ask` skills. Read
...
- If `lore/topics.md` does NOT exist: create the `lore/` directory (plus an empty
  `INDEX.md` if missing) and write the confirmed vocabulary as `lore/topics.md`
