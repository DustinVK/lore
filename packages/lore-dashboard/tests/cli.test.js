...
const BIN = resolve(__dirname, '..', 'bin', 'lore-dashboard.js');
const FIXTURES_LORE = resolve(__dirname, 'fixtures', 'lore');
...
function uniqueTemp(label) {
  return resolve(tmpdir(), `lore-dashboard-test-${label}-${Date.now()}`);
}
...
test('build with missing src exits 1', () => {
  const nonexistent = resolve(tmpdir(), `lore-no-such-${Date.now()}`);
...
test('build happy path produces expected output files', { timeout: 60000 }, () => {
  ...
  try {
    const result = spawn(['build', '--src', FIXTURES_LORE, '--out', out], { timeout: 60000 });
    assert.equal(result.status, 0);
